...
public class CancelCheckoutAction implements Action {
	public WebView execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try{
			...
			if(document == null){
				request.setAttribute(HTTPConstants.REQUEST_ERROR, "Document does not exist");
				return (new CancelDocumentCheckoutView(request,response));
			}
			...
			revisionManager.cancelCheckOut(checkedOutDocument, document);
			//add to audit log
			...
			request.setAttribute(HTTPConstants.REQUEST_MESSAGE,"Document check out cancelled successfully");
		}catch(Exception e){
			e.printStackTrace();
		}
		return (new CancelDocumentCheckoutView(request,response));
